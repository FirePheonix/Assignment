...
class NitterClient:
    ...
    def _find_working_instances(self):
        ...
        for instance in self.NITTER_INSTANCES:
            try:
                ...
                logger.warning(
                    f"Failed to connect to Nitter instance {instance}: {str(e)}"
                )
                continue
    ...
    def get_profile_stats(self):
        ...
        if not self.base_url:
            ...
            if not self.base_url:
                ...
                return self._get_error_stats(
                    "No working Nitter instance available - using fallback data"
                )
        ...
        while retry_count < max_retries:
            try:
                logger.debug(
                    f"Attempt {retry_count + 1}: Fetching stats from {self.base_url}"
                )
                ...
                if retry_count < max_retries:
                    ...
                    if new_base_url:
                        ...
                        logger.debug(
                            f"Switching to alternate instance: {self.base_url}"
                        )
                    else:
    ...
    def _parse_profile_stats(self, html_content):
        ...
        try:
            ...
            website_pattern = (
                r'class="profile-website"[^>]*>(?:<a[^>]*href="([^"]+)"[^>]*>)?([^<]+)'
            )
            website_match = re.search(website_pattern, html_content)
            if website_match:
                stats["website"] = (
                    website_match.group(1)
                    if website_match.group(1)
                    else website_match.group(2)
                )
            ...
            if joined_match:
                try:
                    stats["joined"] = datetime.strptime(
                        joined_match.group(1).strip(), "%B %Y"
                    )
                except ValueError:
                    pass
            ...
            followers_pattern = (
                r'class="followers"[^>]*>.*?class="profile-stat-num"[^>]*>([^<]+)'
            )
            ...
            following_pattern = (
                r'class="following"[^>]*>.*?class="profile-stat-num"[^>]*>([^<]+)'
            )
            ...
            tweets_pattern = (
                r'class="tweets"[^>]*>.*?class="profile-stat-num"[^>]*>([^<]+)'
            )
            ...
            if tweet_date_match:
                try:
                    stats["last_tweet"] = datetime.strptime(
                        tweet_date_match.group(1).split(" · ")[0], "%b %d, %Y"
                    )
    ...
    if hasattr(settings, "TWITTER_USERNAME"):
        ...
        if x_stats:
            stats["x"] = {
                ...
                "status": "danger"
                if x_stats.get("error")
                else "success"
                if x_stats.get("last_tweet")
                else "neutral",
