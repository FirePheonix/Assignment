...
import { NodeDropzoneProvider } from '@/providers/node-dropzone-provider';
...
import { MouseModeToggle } from './mouse-mode-toggle';

export const Canvas = ({ children, readOnly, ...props }: ReactFlowProps & { readOnly?: boolean }) => {
  ...
  // Update nodes and edges when workspace changes
  useEffect(() => {
    if (project?.content) {
      const newContent = project.content as { nodes: Node[]; edges: Edge[] };
      setNodes(newContent.nodes ?? []);
      setEdges(newContent.edges ?? []);
    }
  }, [project?.id]);
  ...
  const handleConnect = useCallback<OnConnect>(
    (connection) => {
      ...
      console.log('🔗 NODE CONNECTION MADE:', {
        source: {
          id: sourceNode?.id,
    ...
    [readOnly, save, onConnect, getNode]
  );
  ...
  const isValidConnection = useCallback<IsValidConnection>(
    (connection) => {
      ...
      if (connection.source) {
        ...
        if (!valid) {
          console.log('❌ INVALID CONNECTION: Invalid source-target pair', {
            source: source.type,
            target: target.type,
            reason: 'isValidSourceTarget returned false',
          });
          return false;
        }
        
        console.log('✅ VALID CONNECTION CHECK:', {
      ...
      <NodeDropzoneProvider>
        <ContextMenu>
          <ContextMenuTrigger onContextMenu={handleContextMenu}>
            <div className={`h-full w-full ${mouseMode === 'pan' ? 'cursor-grab active:cursor-grabbing' : 'cursor-default'}`}>
              <ReactFlow
                ...
                panOnScroll={mouseMode === 'pan'}
                fitView
                zoomOnDoubleClick={false}
                panOnDrag={mouseMode === 'pan'}
                selectionOnDrag={mouseMode === 'select' && !readOnly}
                onDoubleClick={readOnly ? undefined : addDropNode}
                nodesDraggable={mouseMode === 'select' && !readOnly}
                nodesConnectable={mouseMode === 'select' && !readOnly}
                elementsSelectable={mouseMode === 'select'}
                {...rest}
              >
                <Background />
                <MouseModeToggle />
